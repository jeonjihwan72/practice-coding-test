...
편의상 가로수의 위치는 기준점으로부터 떨어져 있는 거리로 표현되며, 가로수의 위치는 모두 양의 정수이다.
...
둘째 줄부터 N개의 줄에는 각 줄마다 심어져 있는 가로수의 위치가 양의 정수로 주어지며, 가로수의 위치를 나타내는 정수는 1,000,000,000이하이다.
가로수의 위치를 나타내는 정수는 모두 다르고, N개의 가로수는 기준점으로부터 떨어진 거리가 가까운 순서대로 주어진다.
'''

# 출력
'''
모든 가로수가 같은 간격이 되도록 새로 심어야 하는 가로수의 최소수를 첫 번재 줄에 출력한다.
'''
...
for i in range(0,N-1):
    length.append(trees[i+1]-trees[i])

GCD = ft.reduce(math.gcd, length)

tree_count = 0
for cnt in length:
    tree_count += (cnt//GCD) - 1

print(f"필요한 가로수 개수: {tree_count}개")