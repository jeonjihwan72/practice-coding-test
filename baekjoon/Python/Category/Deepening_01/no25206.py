# 문제 
'''
인하대학교 컴퓨터공학과를 졸업하기 위해서는, 전공 평점이 3.3이상이거나 졸업고사를 통과해야 한다.
...
원본 링크 : https://www.acmicpc.net/problem/25206
'''
...
for _ in range(20):
    name, _point, _grade = input('과목, 학점, 등급: ').split()
    if _grade == 'P':
        continue
    _point = float(_point)
    grade_score = grade[_grade]
    score += _point * grade_score
    point += _point
