...
1번째 사람은 1의 배수번째 창문이 열려 있으면 닫고, 닫혀있으면 연다.
2번째 사람은 2의 배수번째 창문이 열려 있으면 닫고, 닫혀있으면 연다.
...
opened_window = [False for _ in range(N)]

for i in range(1,N+1):
    for j in range(1, N+1):
        if j % i == 0:
            if opened_window[j-1] == True:
                opened_window[j-1] = False
            else:
                opened_window[j-1] = True

count = 0
for i in range(0, N):
    if opened_window[i] == True:
        count += 1
