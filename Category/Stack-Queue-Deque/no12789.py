...
그런데 이게 무슨 날벼락인가! 그곳에는 이미 모든 학생들이 모여있었고, 승환이는 마지막 번호표를 받게 되었다.
...
만약 불가능하다면 승환이는 이번 중간고사를 망치게 될 것이고 가능하다면 힘을 얻어 중간고사를 잘 볼 수 있을지도 모른다.

사람들은 현재 1열로 줄을 서있고, 맨 앞의 사람만 이동이 가능하다. 
인규는 번호표 순서대로만 통과할 수 있는 라인을 만들어 두었다.
이 라인과 대ㅣㄱ열의 맨 앞 사람 사이에는 한 사람씩 1열이 들어갈 수 있는 공간이 있다.
...
원본 링크 : https://www.acmicpc.net/problem/12789
'''

# 입력
'''
입력의 첫째 줄에는 현재 승환이의 앞에 서있는 학생들의 수 N(1 <= N <= 1,000, 자연수)이 주어진다.
...
def can_get_snack(n, students):
    stack = []  # 보조 공간 역할을 할 스택
    expected = 1  # 현재 간식을 받아야 하는 번호

    for student in students:
        while stack and stack[-1] == expected:  # 보조 공간에서 꺼낼 수 있는 경우
            stack.pop()
            expected += 1

        if student == expected:  # 바로 간식을 받을 수 있는 경우
            expected += 1
        else:  # 간식을 받을 수 없으므로 보조 공간으로 이동
...
# 입력 받기
n = int(input())  # 학생 수
students = list(map(int, input().split()))  # 학생들의 번호표 순서

# 결과 출력
print(can_get_snack(n, students))