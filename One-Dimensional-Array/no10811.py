...
basket = [i for i in range(1, N+1, 1)]

for _ in range(M):
    start, finish = map(int, input("시작 지점과 끝 지점: ").split())
    start, finish = start -1, finish -1
    temp_list = basket[start:finish+1]
    # temp_list.reverse()
    # basket[start:finish+1] = temp_list[:]
    basket[start:finish+1] = temp_list[::-1]
