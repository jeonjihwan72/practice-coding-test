...
A_count, B_count = map(int, input("A, B: ").split())
A = list(map(int, input("A의 원소 입력: ").split()))
if A_count != len(A):
    print("A의 개수가 맞지 않게 입력되었음")
    exit()
    
B = list(map(int, input("B의 원소 입력: ").split()))
if B_count != len(B):
    print("B의 개수가 맞지 않게 입력되었음")
    exit()

A_minus_B = []
for a_element in A:
    if a_element not in B:
        A_minus_B.append(a_element)

B_minus_A = []
for b_element in B:
    if b_element not in A:
        B_minus_A.append(b_element)

total_amount = len(A_minus_B) + len(B_minus_A)
