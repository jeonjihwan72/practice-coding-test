...
원본 링크 : https://www.acmicpc.net/problem/1620
'''
...
다음 줄부터 총 M개의 줄에 내가 맞춰야하는 문제가 입력으로 들어와. 문제가 알파벳으로만 들어오면 포켓몬 번호를 말해야 하고, 숫자로만 들어오면, 포켓몬 번호에 해당하는 문자를 출력해야해.
...
for _ in range(N):
    pocketmonster = input()
    pocketmon.append(pocketmonster)

find_pocket = []
for _ in range(M):
    pocketmonster = input()
    find_pocket.append(pocketmonster)

print("##### 결과 출력 #####")
for check in find_pocket:
    if check.isdigit():
        print(pocketmon[int(check)-1])
    else:
        for i in range(0, N):
            if pocketmon[i] == check:
                print(i+1)